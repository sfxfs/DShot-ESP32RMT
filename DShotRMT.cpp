...
DShotRMT::DShotRMT(gpio_num_t gpio, dshot_mode_t dshot_mode)
{
    ...
    if (dshot_mode >= DSHOT300_BIDIRECTIONAL && dshot_mode <= DSHOT1200_BIDIRECTIONAL)
    {
        ...
        telemetry_bit_len_ticks = (unsigned int)(((float)DSHOT_RMT_RESOLUTION_HZ / baudrate) * 4 / 5);
    }
    ...
    if (is_bidirectional)
    {
        ...
        ESP_ERROR_CHECK(rmt_rx_register_event_callbacks(rmt_rx_channel, &rx_callbacks, rx_queue));
...
uint32_t DShotRMT::sendThrottle(uint16_t throttle_value)
{
    ...
    if (throttle_value > DSHOT_THROTTLE_MIN && is_bidirectional)
    {
        erpm = receive();
    }
 ...
 */
static uint32_t IRAM_ATTR extractTelemetryGcr(rmt_symbol_word_t *rmt_symbols, size_t symbol_num, uint32_t bit_len_ticks)
{
 ...
 */
static uint32_t IRAM_ATTR convertGcrToErpmData(uint32_t value)
{
 ...
 * @return uint32_t
 */
static uint32_t IRAM_ATTR convertErpmDataToErpmPeriod(uint32_t value)
{
    // eRPM range
    if (value == 0x0fff)
    {
...
uint32_t DShotRMT::receive()
{
    if (!enabled || !is_bidirectional)
        return INVALID_TELEMETRY_VALUE;

    uint32_t erpm = INVALID_TELEMETRY_VALUE;
    rmt_rx_done_event_data_t rx_data;
    // ESP_ERROR_CHECK(rmt_receive(rmt_rx_channel, rx_buf, sizeof(rx_buf), &rx_config));
    if (xQueueReceive(rx_queue, &rx_data, pdMS_TO_TICKS(0.5)) == pdPASS)
    {
        // parse the received symbols
        auto gcr = extractTelemetryGcr(rx_data.received_symbols, rx_data.num_symbols, telemetry_bit_len_ticks);
        // ESP_LOGI(TAG, "GCR: %lu", gcr);
        auto erpmData = convertGcrToErpmData(gcr);
        // ESP_LOGI(TAG, "eRPM Data : %lu", erpmData);
        erpm = convertErpmDataToErpmPeriod(erpmData);
        // ESP_LOGI(TAG, "Erpm : %lu", valErpm);
    }
    else
    {
        telemetry_errors++;
...
bool IRAM_ATTR DShotRMT::rxDoneCallback(rmt_channel_handle_t channel, const rmt_rx_done_event_data_t *edata, void *user_data)
{
...
bool IRAM_ATTR DShotRMT::txDoneCallback(rmt_channel_handle_t channel, const rmt_tx_done_event_data_t *edata, void *user_data)
{
    BaseType_t high_task_wakeup = pdFALSE;
